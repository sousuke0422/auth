...
describe('api/v0/auth - OIDC認証フロー', () => {
  ...
  it('プロバイダーがデータベースに存在しない場合でもクライアントがあれば動作する', async () => {
    const prisma = (await import('~/lib/prisma')).default
    vi.mocked(prisma.providers.findFirst).mockResolvedValue(null)
  ...
  it('大文字のプロバイダー名が小文字に正規化される', async () => {
    const prisma = (await import('~/lib/prisma')).default
    vi.mocked(prisma.providers.findFirst).mockResolvedValue({
      id: 'p1',
      name: 'GOOGLE',
  ...
  it('code_verifierがHttpOnly属性でCookieに設定される', async () => {
    const prisma = (await import('~/lib/prisma')).default
    vi.mocked(prisma.providers.findFirst).mockResolvedValue({
      id: 'p1',
      name: 'Google',
