...
describe('api/v0/callback - OIDCコールバックフロー', () => {
  ...
  it('code_verifierが無い場合undefinedが渡される', async () => {
    const prisma = (await import('~/lib/prisma')).default
    ...
    const { callback } = await import('~/api/v0/callback')
    const req = new Request('http://localhost/google?code=code')
  ...
  it('expires_inが0の場合でも処理される', async () => {
    const prisma = (await import('~/lib/prisma')).default

    mockCallback.mockResolvedValue({
      access_token: 'at', id_token: 'it', refresh_token: 'rt', expires_in: 0,
    })
    vi.mocked(prisma.providers.findFirst).mockResolvedValue({
      id: 'p1', name: 'Google', url: 'https://accounts.google.com',
      clientId: 'cid', clientSecret: 'cs', scope: 'openid', isMisskey: false,
    ...
    vi.mocked(prisma.userIdentity.update).mockResolvedValue({
      id: 'i1', expiresAt: new Date(),
    } as any)
  ...
  it('refresh_tokenが無い場合でも処理される', async () => {
    const prisma = (await import('~/lib/prisma')).default

    mockCallback.mockResolvedValue({
      access_token: 'at', id_token: 'it', refresh_token: undefined, expires_in: 3600,
    })
    vi.mocked(prisma.providers.findFirst).mockResolvedValue({
      id: 'p1', name: 'Google', url: 'https://accounts.google.com',
      clientId: 'cid', clientSecret: 'cs', scope: 'openid', isMisskey: false,
